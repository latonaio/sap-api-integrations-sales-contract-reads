...
func (c *SAPAPICaller) callSalesContractSrvAPIRequirementHeader(api, salesContract string) ([]sap_api_output_formatter.Header, error) {
	...
	param := c.getQueryWithHeader(map[string]string{}, salesContract)
...
func (c *SAPAPICaller) callSalesContractSrvAPIRequirementItem(api, salesContract, salesContractItem string) ([]sap_api_output_formatter.Item, error) {
	...
	param := c.getQueryWithItem(map[string]string{}, salesContract, salesContractItem)
...
func (c *SAPAPICaller) getQueryWithHeader(params map[string]string, salesContract string) map[string]string {
	if len(params) == 0 {
		params = make(map[string]string, 1)
	}
	params["$filter"] = fmt.Sprintf("SalesContract eq '%s'", salesContract)
	return params
}

func (c *SAPAPICaller) getQueryWithItem(params map[string]string, salesContract, salesContractItem string) map[string]string {
	if len(params) == 0 {
		params = make(map[string]string, 1)
	}
	params["$filter"] = fmt.Sprintf("SalesContract eq '%s' and SalesContractItem eq '%s'", salesContract, salesContractItem)
	return params
}
